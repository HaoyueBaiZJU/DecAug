...
class IRM_adv(ERM):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(IRM_adv, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.register_buffer('update_count', torch.tensor([0]))
        self.atk = torchattacks.PGD(self.network.cuda(), eps = 8/255, alpha = 2/255, steps=4)
    ...
    def update(self, minibatches):
        ...
        acc = mean_accuracy(all_logits,all_y)
        all_logits_idx = 0
        for i, (x, y) in enumerate(minibatches):
