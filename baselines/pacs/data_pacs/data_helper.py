...
from data_pacs import StandardDataset
from data_pacs.JigsawLoader import JigsawDataset, JigsawTestDataset, get_split_dataset_info, _dataset_info, JigsawTestDatasetMultiple
from data_pacs.concat_dataset import ConcatDataset
...
def get_train_dataset(args):
    ...
    for dname in dataset_list:
        name_train, name_val, labels_train, labels_val = get_split_dataset_info(join(dirname(__file__), 'txt_lists', '%s_train.txt' % dname), 0.1)
...
def get_val_dataset(args):
    ...
    val_dataset = JigsawTestDataset(names, labels, img_transformer=img_tr)
...
def get_train_transformers(args):
    ...
    # img_tr = [transforms.RandomResizedCrop(args.image_size)]
    img_tr.append(transforms.RandomHorizontalFlip(args.random_horiz_flip))
    img_tr.append(transforms.ColorJitter(brightness=args.jitter, contrast=args.jitter, saturation=args.jitter, hue=min(0.5, args.jitter)))
    img_tr += [transforms.RandomGrayscale(0.1),transforms.ToTensor(), transforms.Normalize([0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])]
