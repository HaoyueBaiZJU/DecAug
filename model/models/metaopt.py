...
def MetaOptNetHead_SVM_CS(query, support, support_labels, n_way, n_shot, C_reg=0.1, double_precision=False, maxIter=15):
    ...
    id_matrix_0 = torch.eye(n_way).expand(tasks_per_batch, n_way, n_way)
    if torch.cuda.is_available():
        id_matrix_0 = id_matrix_0.cuda()
    block_kernel_matrix = batched_kronecker(kernel_matrix, id_matrix_0)
    #This seems to help avoid PSD error from the QP solver.
    if torch.cuda.is_available():
        block_kernel_matrix += 1.0 * torch.eye(n_way*n_support).cuda().expand(tasks_per_batch, n_way*n_support, n_way*n_support)
    else:
        block_kernel_matrix += 1.0 * torch.eye(n_way*n_support).expand(tasks_per_batch, n_way*n_support, n_way*n_support)
    ...
    if torch.cuda.is_available():
        ...
        A = Variable(batched_kronecker(id_matrix_2, torch.ones(tasks_per_batch, 1, n_way)))
        b = Variable(torch.zeros(tasks_per_batch, n_support))
        #print (A.size(), b.size())
        if double_precision:
...
class MetaOpt(nn.Module):
    ...
    def forward(self, x, concept_label_set=None, selected_concepts=None):
        ...
        num_tasks = support.shape[0]
        support = support.view(num_tasks, -1, emb_dim)
        query = query.view(num_tasks, -1, emb_dim)

        support_labels = torch.arange(self.args.way, dtype=torch.int16).repeat(self.args.shot).type(torch.LongTensor)
        support_labels = support_labels.unsqueeze(0).repeat(num_tasks, 1)
        if torch.cuda.is_available():
            support_labels = support_labels.cuda()
        logits = self.scale * self.head(query, support, support_labels, self.args.way, self.args.shot, C_reg=self.args.svm_c)
